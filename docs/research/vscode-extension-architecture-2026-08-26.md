...
**[仓库事实]** C2 `ui` descriptor 在 manifest 中静态存在，但 runtime commands 要等 `initialize` 返回才注册；manifest 没有完整的 runtime command declaration：[Plugin Standard](../plugin-standard.md)、[Plugin Protocol](../plugin-protocol.md)。
