...
- [x] Root instructions, README, PR handoff, CI, and release automation point to one workflow.
- [x] The superseded bootstrap Artifact and legacy Eval no longer exist; product change history is
  retained.
- [x] A project-native Bun/TypeScript checker enforces lifecycle shape, readiness, verification,
  release, Incident, Eval, single-source, and branch-diff gates.
- [x] An isolated dry-run proves both the accepted path and representative failure paths.
- [x] The live repository passes the replacement checker and focused tests.
- [x] External branch protection and production monitoring are reported accurately rather than
...
- `bun test script/check-sdlc.test.ts` passed all 10 tests with 24 assertions. It covers valid
...
- An isolated temporary worktree applied the complete repository diff, committed it over base
  `e3744874`, and passed `bun script/check-sdlc.ts --base e3744874`; the worktree was removed after
  its dry-run.
- `bun script/check-sdlc.ts --release-change change-2026-08-30-ai-native-sdlc-migration` failed as
