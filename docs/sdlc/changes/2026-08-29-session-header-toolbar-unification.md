...
- [x] In dark and light appearance, every enabled resting titlebar icon has the same computed
      neutral foreground and transparent background.
- [x] Open or pressed state remains discoverable through a neutral surface without an accent-color
      icon; disabled state remains distinct.
- [x] Environment, pane, project-action, split-menu, plugin, and panel controls share 28px height,
      4px spacing between independent controls, and aligned icon sizing.
- [x] Add action, Open, Commit, split menus, pane controls, environment, plugin action, and panel
      actions retain their accessible names and behavior.
- [x] Focused tests, design check, SDLC check, and rendered console check pass.
