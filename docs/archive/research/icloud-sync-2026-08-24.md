...
**[源码审计，按 2026-08-31 目录更新]** 当前 Rust Core 的 [`device_sync.rs`](../../../crates/core/src/device_sync.rs) 负责同步 document、merge、tombstone 与 SQLite snapshot/import；桌面 Rust host 的 [`device_sync.rs`](../../../apps/desktop/src-host/src/device_sync.rs) 负责 peer 凭据、调度和传输， [`remote.rs`](../../../apps/desktop/src-host/src/remote.rs) 把 `c2` 协议接入现有 remote listener。同步不会另起第二个数据库所有者。用户开启网络访问后 listener 状态和端口会持久化，正常退出不把它误写成关闭。
...
- 桌面平台层解析应用数据目录，Rust host 在该目录打开唯一的 `codetwo.db`：[host startup](../../../apps/desktop/src-host/src/lib.rs) · [Rust store](../../../crates/core/src/store.rs)。
