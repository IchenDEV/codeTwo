...
Scenes standard lives at `docs/reference/scenes.md` + `crates/core/schemas/agent-scenes/1.0.0/`; the
...
R14 as plugin scene/pipeline components plus SKILL.md export (docs/reference/scenes.md §Starter packs).
...
Implement Agent Scenes 1.0.0 per `docs/reference/scenes.md`: loader (`crates/core/src/scene.rs`, modeled on
...
documents, per the UI contract in `docs/reference/scenes.md`. Depends on R1's slot-field component and R3.
...
skill-only hosts as specified in `docs/reference/scenes.md` §Interop.
