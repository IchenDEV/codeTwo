...
use codetwo_core::github_skills::GitHubCheckout;
use codetwo_core::harness::parse_frontmatter;
...
pub fn from_local(
    ...
    let checkout = GitHubCheckout {
        root: temporary,
        spec: codetwo_core::github_skills::GitHubRepoSpec {
            owner: "local".into(),
    ...
    for path in paths {
        ...
        if name.ends_with(".scene.json") {
            let parsed = std::fs::read_to_string(&path)
                ...
                .and_then(|data| {
                    serde_json::from_str::<codetwo_core::scene::Scene>(&data)
                        .map_err(|e| e.to_string())
                })
                .and_then(|scene| codetwo_core::scene::validate_scene(&scene).map(|()| scene));
            match parsed {
                Ok(_) => scenes += 1,
            ...
            let parsed = std::fs::read_to_string(&path)
                .map_err(|e| e.to_string())
                .and_then(|data| {
                    serde_json::from_str::<codetwo_core::scene::Pipeline>(&data)
                        .map_err(|e| e.to_string())
                })
                .and_then(|pipeline| {
                    codetwo_core::scene::validate_pipeline(&pipeline).map(|()| pipeline)
                });
            match parsed {
                Ok(_) => pipelines += 1,
