...
use codetwo_core::keymap::Keymap;
use codetwo_core::memory::MemoryCapability;
use codetwo_core::models::available_models;
...
use codetwo_core::skill::{builtin_skills, Skill, SkillLibrary};
...
/// Revocable product memory published through the plugin kernel without coupling Core to it.
#[derive(Clone)]
...
pub struct TerminalService {
    pub(crate) terminals: Mutex<HashMap<String, codetwo_core::term::TerminalHandle>>,
...
impl ProviderService {
    ...
    pub fn computer_use_settings(&self) -> codetwo_core::host_tools::ComputerUseSettings {
        self.host_tools.read().unwrap().computer_use_settings()
    }

    pub fn browser_use_settings(&self) -> codetwo_core::host_tools::BrowserUseSettings {
...
impl SkillService {
    ...
    pub fn reload(&self, cwd: Option<&Path>) {
        ...
        if let Ok(plugins) = crate::bundle::load_dir(&self.paths.plugins()) {
        ...
        for skill in codetwo_core::harness::discover(cwd.as_deref()) {
            source_enabled.insert(skill.id.clone(), true);
            skills.push(skill);
        }
...
impl SceneService {
    ...
    pub fn reload(&self, cwd: Option<&Path>, hub: Option<&PluginHub>) {
        ...
        let user_dir =
            codetwo_core::provider::home_dir().map(|home| home.join(".config/codetwo/scenes"));
...
pub struct SceneRuntimeService(pub Arc<codetwo_core::scene_runtime::SceneRuntime>);
...
impl std::ops::Deref for SceneRuntimeService {
    type Target = codetwo_core::scene_runtime::SceneRuntime;
...
pub struct CostService(pub Arc<codetwo_core::cost::SessionCostTracker>);
...
impl std::ops::Deref for CostService {
    type Target = codetwo_core::cost::SessionCostTracker;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
...
pub struct HandoffService(pub Arc<codetwo_core::handoff::TaskHandoffManager>);
...
impl std::ops::Deref for HandoffService {
    type Target = codetwo_core::handoff::TaskHandoffManager;
    fn deref(&self) -> &Self::Target {
        &self.0
    }
}
...
impl KeymapService {
    ...
    pub fn set(
        &self,
        action: codetwo_core::keymap::Action,
        key: String,
    ) -> std::io::Result<Keymap> {
...
/// [`crate::bundle::InstalledPlugin`] is a separately managed extension Bundle. This Core service
...
impl PluginHub {
    pub fn installed(&self) -> Vec<crate::bundle::InstalledPlugin> {
        crate::bundle::load_dir(&self.dir).unwrap_or_default()
    }
    ...
    pub fn scene_dirs(&self) -> Vec<(String, PathBuf)> {
        self.installed()
            ...
            .map(|plugin| {
                let dir = crate::bundle::plugin_scenes_dir(&self.dir, &plugin.id);
                (plugin.id, dir)
            })
