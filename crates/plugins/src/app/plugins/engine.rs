...
use codetwo_core::engine::{Engine, ParallelTaskCreation};
use codetwo_core::event::Op;
use codetwo_core::permission::{ExecutionPolicy, PermissionMode, SandboxPolicy};
use codetwo_core::provider::ProviderId;
use codetwo_core::session::TranscriptCursor;
...
#[derive(Clone)]
struct QueuedPrompt {
    doc: Vec<codetwo_core::skill::DocBlock>,
...
pub struct EngineInputs {
    pub providers: Vec<codetwo_core::provider::Provider>,
    pub provider_tools: Arc<
        std::sync::RwLock<
            std::collections::HashMap<String, codetwo_core::provider::ProviderToolset>,
        >,
    >,
    pub skills: codetwo_core::skill::SkillLibrary,
    pub store: Arc<codetwo_core::store::Store>,
    pub memory: Option<codetwo_core::memory::MemoryCapability>,
}
...
pub type EngineBuilder = Arc<
    dyn Fn(
            ...
            Engine,
            tokio::sync::mpsc::UnboundedReceiver<codetwo_core::event::Event>,
        ) + Send
        + Sync,
>;
...
impl Plugin for EnginePlugin {
    ...
    async fn apply(&self, ctx: Context, _config: Value) -> PluginResult {
        ...
        let inputs = EngineInputs {
            ...
            store: store.0.clone(),
            memory: ctx
                .get::<crate::app::service::MemoryService>()
                .map(|memory| memory.0.clone()),
        };
        ...
        let worktree_settings = codetwo_core::worktree::load_settings(&paths.data_dir)
            .unwrap_or_else(|error| {
                tracing::warn!("could not load worktree settings: {error}");
                codetwo_core::worktree::WorktreeSettings::default()
            });
...
fn register_commands(
            ...
            json(
                engine
                    .transcript_page(
                        ...
                        args.before,
                        args.limit
                            .unwrap_or(codetwo_core::session::DEFAULT_TRANSCRIPT_TURNS),
                    )
                    .map_err(PluginError::new)?,
            )
        }
    ...
    ctx.command("sessions.import", move |args| {
        ...
        async move {
            ...
            let report = codetwo_core::session_import::import_session_files(
                &store,
                &args.paths,
                &args.fallback_cwd,
            );
            for session in report.sessions.iter().filter(|session| session.imported) {
                bus.publish(codetwo_core::event::Event::SessionCreated {
    ...
    ctx.command("engine.new_session", move |args| {
        ...
        async move {
            ...
            let worktree_settings =
                codetwo_core::worktree::load_settings(&settings_dir).map_err(PluginError::new)?;
            ...
            if args.use_worktree && worktree_settings.fetch_upstream {
                ...
                codetwo_core::worktree::fetch_upstream(source)
                    .await
                    .map_err(PluginError::new)?;
                let baseline = args.worktree_base.unwrap_or(WorktreeBaseline::Current);
                args.worktree_base_sha = Some(
                    codetwo_core::worktree::resolve_baseline(source, baseline)
                        .await
                        .map_err(PluginError::new)?
                        .sha,
                );
    ...
    ctx.command("engine.queue", move |args| {
        ...
        async move {
            let args: PromptArgs = take_args(args)?;
            if codetwo_core::skill::canonical_doc_text(&args.doc)
                .trim()
                .is_empty()
            {
            ...
            bus.publish(codetwo_core::event::Event::PromptQueued {
                session: args.session,
                request_id: args.request_id,
                position,
            });
    ...
    ctx.spawn(async move {
        loop {
            ...
            let session = match &event {
                codetwo_core::event::Event::TurnEnded { session, .. } => Some(session.clone()),
                codetwo_core::event::Event::Error {
            ...
            for (index, queued) in remaining.into_iter().enumerate() {
                draining_bus.publish(codetwo_core::event::Event::PromptQueued {
    ...
    ctx.command("worktrees.settings", move |_| {
        ...
        async move {
            json(codetwo_core::worktree::load_settings(&settings_dir).map_err(PluginError::new)?)
        }
    })?;

    #[derive(Deserialize)]
    struct WorktreeSettingsArgs {
        settings: codetwo_core::worktree::WorktreeSettings,
    }
    let settings_dir = paths.data_dir.clone();
    ...
    ctx.command("worktrees.set_settings", move |args| {
        ...
        async move {
            ...
            let settings = codetwo_core::worktree::save_settings(&settings_dir, args.settings)
                .map_err(PluginError::new)?;
