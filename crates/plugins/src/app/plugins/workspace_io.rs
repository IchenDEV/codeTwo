...
use codetwo_core::project::{self, ProjectWorktreeMode};
use codetwo_core::workspace;
use codetwo_core::workspace_search::{self, WorkspaceSearchCancellation, WorkspaceSearchOptions};
    ...
    async fn apply(&self, ctx: Context, _config: Value) -> PluginResult {
        ...
        ctx.command("attachments.import", move |args| {
            ...
            async move {
                ...
                let attachment = codetwo_core::attachment::import_prompt_attachment(
                    &data_dir,
                    &args.name,
        ...
        ctx.command("attachments.get", move |args| {
            ...
            async move {
                ...
                let attachment =
                    codetwo_core::attachment::load_prompt_attachment(&data_dir, &args.id)
                        .map_err(PluginError::new)?;
                json(serde_json::json!({
                    "id": attachment.id,
                    "kind": "attachment",
        ...
        ctx.command("workspace.rules", |args| async move {
            ...
            json(
                codetwo_core::rules::load(Path::new(&args.cwd))
                    .into_iter()
                    .map(|rule| rule.path)
                    .collect::<Vec<_>>(),
            )
        ...
        ctx.command("workspace.source_control", |args| async move {
            let args: CwdArgs = take_args(args)?;
            json(
                codetwo_core::source_control::inspect(Path::new(&args.cwd))
                    .await
                    .map_err(PluginError::new)?,
            )
        })?;
        ...
        struct SaveScriptArgs {
            ...
            name: String,
            #[serde(default)]
            kind: codetwo_core::project::ProjectActionKind,
            #[serde(default)]
            command: String,
            #[serde(default)]
            prompt: String,
        ...
        ctx.command("workspace.save_script", |args| async move {
            ...
            json(project::save_script(
                Path::new(&args.cwd),
                &codetwo_core::project::ProjectScript {
                    id: args.id,
                    name: args.name,
                    kind: args.kind,
...
async fn resolve_baselines(cwd: &Path) -> Vec<WorktreeBaselineOption> {
    let (current, origin_default) = tokio::join!(
        codetwo_core::worktree::resolve_baseline(cwd, WorktreeBaseline::Current),
        codetwo_core::worktree::resolve_baseline(cwd, WorktreeBaseline::OriginDefault),
    );
    [
    ...
    async fn apply(&self, ctx: Context, _config: Value) -> PluginResult {
        ...
        ctx.command("projects.open", move |args| {
            ...
            async move {
                ...
                store
                    .touch_project(&args.path, codetwo_core::session::now_millis())
                    .map_err(PluginError::new)?;
                Ok(Value::Bool(true))
            }
        })?;
        ...
        ctx.command("projects.set_icon", move |args| {
            ...
            async move {
                ...
                let Some(source) = args.source else {
                    let revision = store
                        .set_project_icon(&args.path, None, codetwo_core::session::now_millis())
                        .map_err(PluginError::new)?;
...
fn read_visualization(path: &str, realm: &CommandRealm) -> Result<String, PluginError> {
    ...
    if let Some(home) = codetwo_core::provider::home_dir() {
...
fn home_dir() -> Option<PathBuf> {
    codetwo_core::provider::home_dir()
}
