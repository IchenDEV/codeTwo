...
use codetwo_plugins::bundle::PluginRuntimeCommand;
use codetwo_plugins::protocol::{Channel, ProtocolPlugin, Transport, PROTOCOL_VERSION};
...
async fn an_untrusted_bundle_that_ships_a_process_is_not_started() {
    ...
    app.ctx()
        .emit(codetwo_plugins::events::PluginsChanged)
        .await;
    app.flush().await;
