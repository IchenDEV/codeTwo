...
use crate::plugins::app::service::{CanvasService, EngineService, Paths, SkillService, StoreService};
...
fn register_canvas_commands(ctx: &Context, canvas: Arc<CanvasService>) -> PluginResult {
    ctx.command("canvas.feature_state", |_| async move {
        json(CanvasFeatureStateDto {
            feature: crate::canvas::CANVAS_FEATURE_GATE,
            enabled: false,
    ...
    ctx.command("canvas.normalize_media", move |args| {
        ...
        async move {
            ...
            json(CanvasAssetDto::from(
                crate::canvas::normalize_media(&args.bytes, args.declared_mime.as_deref())
                    .map_err(PluginError::new)?,
            ))
        }
    })?;
            ...
            json(CanvasSnapshotDto::from(
                canvas
                    ...
                    .freeze_canvas_with_gate(
                        ...
                        &canvas.owner,
                        args.expected_revision,
                        args.input.into_core(crate::session::now_millis()),
                    )
                    .map_err(PluginError::new)?,
            ))
        }
    ...
    ctx.command("canvas.tombstone", move |args| {
        ...
        async move {
            ...
            canvas
                .store
                .tombstone_canvas(&args.id, &canvas.owner, crate::session::now_millis())
                .map_err(PluginError::new)?;
            Ok(Value::Bool(true))
        }
    })?;
    ...
    ctx.command("canvas.restore", move |args| {
        ...
        async move {
            ...
            canvas
                .store
                .restore_canvas(&args.id, &canvas.owner, crate::session::now_millis())
                .map_err(PluginError::new)?;
            Ok(Value::Bool(true))
        }
    })?;
    ...
    ctx.command("canvas.purge", move |args| {
        ...
        async move {
            ...
            json(
                canvas
                    .store
                    .purge_canvas(&args.id, &canvas.owner, crate::session::now_millis())
                    .map_err(PluginError::new)?,
            )
        }
    })?;
        ...
        ctx.command("document.compile", move |args| {
            ...
            async move {
                ...
                let compiled = crate::skill::compile_with_canvas(
                    ...
                    Some(&resolve),
                    canvas.gate,
                    crate::canvas::CanvasProviderImageCapability::Unknown,
...
struct CanvasObjectDto {
    id: String,
    kind: crate::canvas::CanvasObjectKind,
    original_text: String,
    bounds: crate::canvas::CanvasRect,
    layer: i64,
    arrow_start: Option<crate::canvas::CanvasPoint>,
    arrow_end: Option<crate::canvas::CanvasPoint>,
...
struct CanvasExportDto {
    id: String,
    kind: crate::canvas::CanvasExportKind,
    index: Option<u32>,
    mime_type: String,
    width: u32,
    height: u32,
