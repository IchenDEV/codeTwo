                ...
                let instance = scopes.iter().find(|instance| {
                    ...
                    instance.plugin == entry.name
                        && match (runtime_project, &instance.command_realm) {
                            (None, crate::kernel::CommandRealm::Global) => true,
                            (Some(project), crate::kernel::CommandRealm::Project(realm)) => {
                                project == realm
                            }
                            _ => false,
                        }
    ...
    pub async fn settle_and_mark_last_good(
        ...
        let Some(settle) = &result.settle else {
            ...
            if result.component_policy_changed {
                context.emit(crate::plugins::app::events::PluginPolicyChanged).await;
            }
            return Ok(());
        };
...
fn affected_plugins(
    target: &str,
    entries: &[crate::kernel::LoaderEntryInfo],
    context: &WeakContext,
    scope: &PluginScope,
) -> Vec<String> {
