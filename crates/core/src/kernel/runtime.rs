...
use crate::kernel::command::{CommandEntry, CommandInfo, CommandRealm, CommandVisibility};
use crate::kernel::error::{KernelError, PluginError};
use crate::kernel::event::{Event, JsonListenerEntry, ListenerEntry};
use crate::kernel::plugin::{Injection, Plugin};
use crate::kernel::service::{AnyService, ServiceInfo};
...
/// Owns the plugin graph. Reached through a [`Context`](crate::kernel::Context); you rarely hold one
...
impl Runtime {
    ...
    pub(crate) fn register_command(
        ...
        handler: crate::kernel::command::CommandHandler,
    ) -> Result<(), KernelError> {
        let mut state = self.state();
    ...
    pub(crate) fn command_handler(
        ...
        realm: &CommandRealm,
        name: &str,
    ) -> Result<Option<crate::kernel::command::CommandHandler>, KernelError> {
        let state = self.state();
    ...
    pub(crate) fn listeners_for<E: crate::kernel::event::Event>(&self) -> Vec<crate::kernel::event::Handler<E>> {
        let state = self.state();
        ...
        entries
            .iter()
            .filter_map(|entry| entry.handler.downcast_ref::<crate::kernel::event::Handler<E>>())
            .cloned()
            .collect()
    }

    pub(crate) fn json_listeners_for(&self, name: &str) -> Vec<crate::kernel::event::JsonHandler> {
        let state = self.state();
...
impl Runtime {
    ...
    async fn unload(self: &Arc<Self>, root: ScopeId, remove: bool) {
        ...
        for (name, realm, scope) in withdrawn {
            self.notify_service_change(&name, realm, scope);
            self.emit_driver_event(crate::kernel::events::ServiceChanged {
                name,
                realm,
                active: false,
            })
    ...
    async fn load(self: &Arc<Self>, id: ScopeId) {
        ...
        let ctx =
            crate::kernel::Context::with_isolate(self.clone(), id, generation, isolate, command_realm);
    ...
    async fn emit_status(
        ...
        self.emit_driver_event(crate::kernel::events::StatusChanged {
            scope,
            plugin,
            status,
            error,
    ...
    async fn emit_driver_event<E: Event>(self: &Arc<Self>, event: E) {
        let ctx = crate::kernel::Context::for_scope(self.clone(), ROOT_SCOPE);
...
async fn drive(runtime: Weak<Runtime>) {
    loop {
        ...
        if first_idle {
            rt.emit_driver_event(crate::kernel::events::Ready).await;
            continue;
        }
