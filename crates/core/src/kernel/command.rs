...
//!
//! So commands are registered on a [`Context`](crate::kernel::Context), owned by its scope, and dispatched
//! by name through [`Context::call`](crate::kernel::Context::call). Unloading the plugin removes them; the
//! bridge learns nothing new. `JSON in, JSON out` is the price of being callable from four
//! runtimes.

use crate::kernel::error::PluginError;
use crate::kernel::event::BoxFuture;
use crate::kernel::runtime::ScopeId;
use serde_json::Value;
use std::sync::Arc;

/// The command namespace a [`Context`](crate::kernel::Context) dispatches through.
///
