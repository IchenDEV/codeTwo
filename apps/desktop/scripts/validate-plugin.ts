...
if (!existsSync(source)) {
  throw new Error(`plugin source was not found: ${source}`);
}
const manifestPath = statSync(source).isDirectory()
  ? join(source, "plugin.json")
  : source;
if (!existsSync(manifestPath)) {
  throw new Error(`plugin.json was not found at ${manifestPath}`);
}
if (lstatSync(manifestPath).isSymbolicLink()) {
  throw new Error("plugin.json must not be a symbolic link");
}
...
if (
  manifest.runtime &&
  (manifest.runtime.command.includes("/") ||
    manifest.runtime.command.includes("\\"))
) {
  if (isAbsolute(manifest.runtime.command)) {
    throw new Error(
      "runtime.command must be an executable name or a bundle-relative path"
    );
  }
  const executable = resolve(root, manifest.runtime.command);
  if (!existsSync(executable)) {
    throw new Error(
      `runtime executable was not found: ${manifest.runtime.command}`
    );
  }
...
for (const server of manifest.languageServers) {
  ...
  if (isAbsolute(server.command)) {
    throw new Error(
      `languageServers.${server.id}.command must be an executable name or a bundle-relative path`
    );
  }
  const executable = resolve(root, server.command);
  if (!existsSync(executable)) {
    throw new Error(
      `language server executable was not found: ${server.command}`
    );
  }
...
const contributionCount =
  ...
  manifest.languageServers.length +
