...
export function CanvasEditorComponent(
  {
    className,
    value,
    mode = "edit",
  ...
  ref: ForwardedRef<CanvasEditorHandle>
) {
  ...
  const onSceneChange = (
    ...
    const filteredElements = sanitizeElements(elements).filter(
      (element) =>
        element.type !== "image" ||
        (element.fileId !== null && knownFileIdsRef.current.has(element.fileId))
    );
    sceneRef.current = {
      appState,
  ...
  const normalizeFiles = async (files: readonly File[]) => {
    ...
    try {
      ...
      const mediaError =
        error instanceof Error ? error : new Error("Canvas media input failed");
      setError(mediaError.message);
      onMediaError?.(mediaError);
    }
  };
  ...
  useEffect(() => {
    ...
    void (async () => {
      const assets = (
        await Promise.all(
          effectiveEnvelope.assetReferences.map((asset) => assetResolver(asset))
        )
      ).filter(
        (
          asset
...
export const CanvasEditor = forwardRef(CanvasEditorComponent);
