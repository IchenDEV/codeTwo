...
import type { AppState, BinaryFiles, ExcalidrawElement } from "./excalidrawAdapter";
import {
...
export { DEFAULT_EXPORT_BUDGET, CanvasExportBudgetError, planCanvasExportTiles } from "./exportPlan";
export type { CanvasExportBounds, CanvasExportBudget, CanvasExportTile } from "./exportPlan";
...
export function getCanvasExportBounds(elements: readonly unknown[], margin = DEFAULT_EXPORT_BUDGET.margin): CanvasExportBounds | null {
  const visible = sanitizeElements(elements).filter((element) => !element.isDeleted && element.opacity > 0);
...
async function canvasBlob(canvas: HTMLCanvasElement): Promise<Blob> {
  return new Promise((resolve, reject) => {
    canvas.toBlob((blob) => (blob ? resolve(blob) : reject(new CanvasExportBudgetError("canvas-unavailable", "Canvas could not produce a PNG"))), "image/png");
  });
}

function makeCanvas(width: number, height: number): HTMLCanvasElement {
  if (typeof document === "undefined") throw new CanvasExportBudgetError("canvas-unavailable", "PNG export requires a browser canvas");
...
export async function exportCanvasPng(
  ...
  if (!bounds) throw new CanvasExportBudgetError("empty-scene", "Cannot export an empty canvas");
  const rendered = await exportToCanvas({
    ...
    appState: { ...appState, viewBackgroundColor: "transparent", gridModeEnabled: false, gridSize: 0 },
    files,
  ...
  if (!sourceContext) throw new CanvasExportBudgetError("canvas-unavailable", "Canvas 2D context is unavailable");
  ...
  for (const tile of tiles) {
    ...
    if (!context) throw new CanvasExportBudgetError("canvas-unavailable", "Canvas 2D context is unavailable");
    if (tile.kind === "overview") {
      context.drawImage(source, 0, 0, source.width, source.height, 0, 0, tile.width, tile.height);
    } else {
      context.drawImage(source, tile.sourceX, tile.sourceY, tile.sourceWidth, tile.sourceHeight, 0, 0, tile.width, tile.height);
    }
    const blob = await canvasBlob(target);
    totalBytes += blob.size;
    if (totalBytes > limits.maxBytes) {
      throw new CanvasExportBudgetError("byte-budget", `Canvas export produced ${totalBytes} bytes; limit is ${limits.maxBytes}`);
    }
