...
export function AppshotsSettingsPage({
  ...
  permissionRequester?: (kind: "screen-recording" | "accessibility") => Promise<AppshotSettings>;
  privacyOpener?: (kind: "screen-recording" | "accessibility") => Promise<boolean>;
  ...
  useEffect(() => {
    ...
    void loader().then((next) => {
      ...
      if (active) setError(t("settings.appshotsLoadFailed", { error: String(cause) }));
    });
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    if (!appshotSettings?.available || (appshotSettings.screen_recording && appshotSettings.accessibility)) return;
    let active = true;
  ...
  }, [loader, appshotSettings?.accessibility, appshotSettings?.available, appshotSettings?.screen_recording]);

  async function save(patch: Partial<Pick<AppshotSettings, "hotkey" | "destination" | "play_sound">>) {
    setSaving(true);
    setError(null);
    try {
  ...
  async function grant(kind: "screen-recording" | "accessibility") {
    ...
    try {
      ...
      setError(t("settings.appshotsPermissionFailed", { error: String(cause) }));
    } finally {
      setSaving(false);
    }
  }
  ...
  const hotkeyLabel = appshotSettings?.hotkey === "both-command"
  ...
  const destinationLabel = appshotSettings?.destination === "automatic"
    ...
    <Page title={t("settings.appshots")} description={t("settings.appshotsHint")}>
      <div className="mb-surface-inset">
        ...
        >
          ...
          >
            {capturing ? t("settings.appshotsCapturing") : t("settings.appshotsTakeNow")}
          </Button>
        </SettingRow>
      </div>
          ...
          <Row label={t("settings.appshotsHotkey")} hint={t("settings.appshotsHotkeyHint")}>
            <Select
              ...
              onValueChange={(hotkey) => {
                if (hotkey) void save({ hotkey: hotkey as AppshotSettings["hotkey"] });
              }}
            >
          ...
          <Row label={t("settings.appshotsDestination")} hint={t("settings.appshotsDestinationHint")}>
            <Select
              ...
              onValueChange={(destination) => {
                if (destination) void save({ destination: destination as AppshotSettings["destination"] });
              }}
            >
...
function PermissionRow({
  ...
  return (
    <Row compact label={label} hint={hint}>
      ...
      <Button variant="ghost" size="sm" className="text-muted-foreground" onClick={onOpen}>
