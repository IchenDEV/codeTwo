...
type HostCall = (name: string, args: unknown, projectPath: string | null) => Promise<unknown>;
...
function activeContributions(installed: unknown, catalog: unknown): HostActionContribution[] {
  ...
  for (const value of installed) {
    ...
    if (!plugin || typeof plugin.id !== "string" || plugin.enabled !== true || plugin.trusted !== true) {
      continue;
    }
    const policy = policies.get(`bundle:${plugin.id}`);
    if (policy && (policy.enabled !== true || policy.running !== true)) continue;
    ...
    for (const value of plugin.ui_contributions) {
      ...
      if (!contribution || contribution.slot !== HOST_ACTION_SLOT ||
          typeof contribution.id !== "string" || typeof contribution.label !== "string" ||
          typeof contribution.order !== "number") continue;
      const components = asObject(policy?.components);
      if (components?.[`bundle:${plugin.id}:ui:${contribution.id}`] === "disabled") continue;
      contributions.push({
        pluginId: plugin.id,
...
function parseDocument(value: unknown): ActionDocumentItem[] | null {
  ...
  const allowed = new Set([
    "id", "label", "detail", "state", "enabled", "input", "accessibilityLabel",
  ]);
  for (const value of rawItems) {
    const item = asObject(value);
    if (!item || Object.keys(item).some((key) => !allowed.has(key))) return null;
    if (typeof item.id !== "string" || !/^[A-Za-z0-9_-]{1,128}$/.test(item.id) ||
        ids.has(item.id)) return null;
    if (typeof item.label !== "string" || item.label.trim() === "" ||
        Array.from(item.label).length > 80) return null;
    ...
    if (typeof detail !== "string" || Array.from(detail).length > 80 ||
        !["default", "running", "attention", "failure"].includes(String(state)) ||
        typeof enabled !== "boolean" || typeof accessibilityLabel !== "string" ||
        Array.from(accessibilityLabel).length > 160 ||
        (input !== null && asObject(input) === null)) return null;
...
export class PluginHostActionController {
  ...
  invoke(key: string, itemId: string): void {
    ...
    void this.call("plugins.invoke_ui", {
  ...
  private async refresh(): Promise<void> {
    ...
    try {
      ...
      const rendered = await Promise.allSettled(contributions.map(async (contribution) => ({
        contribution,
        items: parseDocument(await this.call("plugins.invoke_ui", {
          plugin_id: contribution.pluginId,
          contribution_id: contribution.contributionId,
      ...
      this.active = active;
      this.inputs = inputs;
      if (!this.adapter.render(hostItems.slice(0, MAX_ACTIONS))) this.adapter.render([]);
    } catch (error) {
      if (generation !== this.generation) return;
      this.active.clear();
      this.inputs.clear();
