...
export function nativeContextMenuConfig(
  ...
  return items.map((item) => {
    ...
    return {
      ...
      ...(item.submenu ? { submenu: nativeContextMenuConfig(item.submenu, requestId) } : {}),
    };
  });
}

export function nativeContextMenuAction(event: unknown): NativeContextMenuAction | null {
  ...
  if (action !== CONTEXT_MENU_ACTION || typeof data !== "object" || data === null) return null;

  const requestId = (data as { requestId?: unknown }).requestId;
  const selectedAction = (data as { action?: unknown }).action;
  if (typeof requestId !== "string" || typeof selectedAction !== "string") return null;
