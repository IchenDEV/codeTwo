...
export function useTaskBoardActions(options: TaskBoardActionsOptions) {
  const openEditor = (task: BoardTask | null, initialStatus: TaskStatus): void => {
  ...
  const deleteTask = async (task: BoardTask): Promise<void> => {
    const confirmed = await confirmNative(options.t("taskboard.deleteConfirm", { title: task.title }))
    if (!confirmed) return
    options.dispatch({ type: "delete", id: task.id })
    options.toast(options.t("taskboard.deleted", { title: task.title }), "success")
  }
