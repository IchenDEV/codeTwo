...
export const PULL_REQUEST_TONES: Record<
  SidebarPullRequestStatus["state"],
  string
> = {
  merged: "text-success",
...
export function sessionActivityKind(
...
export function sessionActivityDescription(
  t: Translate,
  session: SessionProjection
): string {
...
function taskSessions(
  ...
  sessionsById: ReadonlyMap<
    string,
    Omit<SessionProjection, "number" | "current">
  >
): SessionProjection[] {
  const oldestFirst: Omit<SessionProjection, "current">[] =
    task.sessionIds.flatMap((id, index) => {
