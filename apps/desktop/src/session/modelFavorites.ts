...
export function loadModelFavorites(
  ...
  try {
    ...
    for (const [provider, favorites] of Object.entries(candidate.providers).slice(0, MAX_PROVIDERS)) {
      if (!isSafeProviderKey(provider) || !Array.isArray(favorites)) continue
      providers[provider] = [
        ...new Set(favorites.filter(isSafeKey).slice(0, MAX_FAVORITES_PER_PROVIDER)),
      ]
    }
    return { version: MODEL_FAVORITES_VERSION, providers }
  } catch {
...
export function toggleModelFavorite(
  ...
  if (!isSafeProviderKey(provider) || !isSafeKey(model)) return favoritesForProvider(provider, storage)
...
export function useProviderModelFavorites(provider: string) {
  const [favorites, setFavorites] = useState(() => favoritesForProvider(provider))
  ...
  const toggle = useCallback(
    (model: string) => {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(new CustomEvent<ModelFavoritesChangeDetail>(MODEL_FAVORITES_EVENT, {
