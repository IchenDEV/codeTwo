...
export function paneInDirection(
  ...
  for (const [id, r] of rects) {
    ...
    switch (direction) {
      case "left": {
        isInDir = cx < fromCx - 1e-6;
        isPerp = rangesOverlap(from.y, from.h, r.y, r.h);
      ...
      case "top": {
        isInDir = cy < fromCy - 1e-6;
        isPerp = rangesOverlap(from.x, from.w, r.x, r.w);
