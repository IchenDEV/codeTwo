...
function CatalogCard({ card }: { card: UiLabCard }) {
  ...
  return (
    <a className="ui-lab-card" href={uiLabHref(card.route, preference, locale)}>
      <span className="ui-lab-card-icon"><Icon className="size-4" /></span>
      ...
      <span className="ui-lab-card-footer">
        ...
        {card.fixture ? <span className="ui-lab-fixture-badge">Fixture</span> : null}
      </span>
    </a>
  );
}
...
function LocaleLinks({ route }: { route: UiLabRoute }) {
  ...
  const { locale } = useLanguage();
  return (
    <div className="ui-lab-control-group ui-lab-locale-links" aria-label="Preview language">
      <Globe className="size-3.5" aria-hidden="true" />
      <a aria-current={locale === "en" ? "true" : undefined} href={uiLabHref(route, preference, "en")}>EN</a>
      <a aria-current={locale === "zh-CN" ? "true" : undefined} href={uiLabHref(route, preference, "zh-CN")}>中文</a>
    </div>
  );
}
      ...
      <ScrollArea className="min-h-0 flex-1">
        <div className="ui-lab-transcript">
          <article className="ui-lab-message ui-lab-message-user">
            <span>YOU</span>
            <p>Can you review the usage widget changes and make sure the narrow layout still works?</p>
          </article>
          <article className="ui-lab-message ui-lab-message-assistant">
            <span>C2</span>
            <p>The implementation is ready for review. I verified the responsive shell and the three desktop checks pass.</p>
            <div className="ui-lab-run-summary"><Code2 className="size-3.5" />3 checks passed · 30 files changed</div>
          </article>
        </div>
      </ScrollArea>
    ...
    <div className="ui-lab-root">
      <aside className="ui-lab-sidebar">
        <div className="ui-lab-brand">
          <span>C2</span>
          <div><strong>UI Lab</strong><small>Development catalog</small></div>
        </div>
        <nav aria-label="UI Lab sections">
          <a aria-current="page" href={uiLabHref("home", preference, locale)}>Catalog</a>
        ...
        <div className="ui-lab-sidebar-note">
          <span className="ui-lab-dev-badge">Dev only</span>
          <p>Fixtures are local, deterministic, and never call a bridge or remote service.</p>
        </div>
      </aside>
      ...
      <main className="ui-lab-main">
        <header className="ui-lab-header">
          <div>
            ...
            <h1>UI Lab</h1>
            <p>Stable production-component scenarios for design review, regression checks, and screenshots.</p>
          </div>
        ...
        <section id="production-scenarios" className="ui-lab-section" aria-labelledby="production-title">
          <div className="ui-lab-section-heading">
            <div><span>01</span><h2 id="production-title">Production scenarios</h2></div>
            <p>These pages render the same components used in C2.</p>
          </div>
          <div className="ui-lab-card-grid">
            {catalogCards.map((card) => <CatalogCard key={card.route} card={card} />)}
          </div>
        </section>

        <section id="existing-previews" className="ui-lab-section" aria-labelledby="previews-title">
          <div className="ui-lab-section-heading">
            <div><span>02</span><h2 id="previews-title">Focused previews</h2></div>
            <p>Existing single-purpose development surfaces remain directly accessible.</p>
          </div>
          <div className="ui-lab-preview-grid">
            {previewCards.map((card) => <PreviewCard key={card.href} card={card} />)}
          </div>
        </section>
      </main>
    </div>
