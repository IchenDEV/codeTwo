...
describe("macOS application menu", () => {
  test("exposes native responder roles for text and image paste", () => {
    const editMenu = macOSApplicationMenu().find((item) =>
      "label" in item && item.label === "Edit"
    );
    const roles = editMenu && "submenu" in editMenu
      ? editMenu.submenu?.flatMap((item) => "role" in item ? [item.role] : [])
      : [];
