...
test("runtime command deadlines are finite and optional", () => {
  expect(parsePluginRuntimeContribution({ command: "node" })?.commandTimeoutMs).toBeUndefined();
  for (const value of [1, 60_000, 3_600_000]) {
    expect(parsePluginRuntimeContribution({ command: "node", commandTimeoutMs: value })?.commandTimeoutMs).toBe(value);
  }
  for (const value of [0, -1, 3_600_001, 1.5, null, "60000"]) {
    expect(parsePluginRuntimeContribution({ command: "node", commandTimeoutMs: value })).toBeNull();
  }
});
