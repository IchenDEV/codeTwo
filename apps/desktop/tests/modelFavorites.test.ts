...
describe("model favorites", () => {
  test("persists unique model identities independently for each provider", () => {
    ...
    expect(toggleModelFavorite("opencode", "model-a", storage)).toEqual(["model-a"])
    expect(toggleModelFavorite("opencode", "model-b", storage)).toEqual(["model-a", "model-b"])
    expect(toggleModelFavorite("cursor", "model-a", storage)).toEqual(["model-a"])
    expect(toggleModelFavorite("opencode", "model-a", storage)).toEqual(["model-b"])

    expect(favoritesForProvider("opencode", storage)).toEqual(["model-b"])
    expect(favoritesForProvider("cursor", storage)).toEqual(["model-a"])
    expect(JSON.parse(storage.getItem(MODEL_FAVORITES_STORAGE_KEY) ?? "null")).toEqual({
      version: 1,
      providers: {
        opencode: ["model-b"],
        cursor: ["model-a"],
  ...
  test("degrades corrupt, outdated, and partially invalid documents safely", () => {
    ...
    storage.setItem(MODEL_FAVORITES_STORAGE_KEY, JSON.stringify({ version: 0, providers: {} }))
    expect(loadModelFavorites(storage).providers).toEqual({})
  ...
  test("keeps an in-memory toggle usable when storage writes fail", () => {
    ...
    expect(toggleModelFavorite("codex", "gpt-5", unavailable)).toEqual(["gpt-5"])
  })
})
