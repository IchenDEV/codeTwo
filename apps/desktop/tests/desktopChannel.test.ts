...
describe("desktop distribution channels", () => {
  test("use distinct bundle identifiers", () => {
    const identifiers = Object.values(DESKTOP_CHANNELS).map(
  ...
  test("infer safe defaults from Electrobun arguments", () => {
    ...
    expect(resolveDesktopChannel(undefined, ["build", "--env=dev"])).toBe(
      "dev"
    );
    expect(resolveDesktopChannel(undefined, ["build", "--env=stable"])).toBe(
      "release"
    );
    expect(resolveDesktopChannel("nightly", ["build", "--env=stable"])).toBe(
      "nightly"
    );
  });
  ...
  test("maps build hook identifiers back to their channel", () => {
    ...
    expect(desktopChannelForIdentifier("dev.codetwo.app.nightly")).toBe(
      "nightly"
    );
  ...
  test("rejects unknown channels", () => {
    expect(() => resolveDesktopChannel("beta")).toThrow(
      "Unsupported C2 desktop channel: beta"
    );
  });
});
