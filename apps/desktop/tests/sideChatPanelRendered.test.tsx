...
describe("SideChatPanel", () => {
  ...
  test("keeps Quick Chat and Side Chat as separately named placement surfaces", async () => {
    ...
    expect(quick.container.querySelector('[role="dialog"]')?.getAttribute("aria-label"))
      .toBe("Quick Chat");
    expect(quick.container.querySelector(".quick-chat-panel")).not.toBeNull();
    expect(side.container.querySelector('[aria-label="Side chat"]')).not.toBeNull();
    expect(side.container.querySelector(".quick-chat-panel")).toBeNull();
    expect(side.container.querySelector('[role="dialog"]')).toBeNull();
    expect(side.container.querySelector("[data-transient-chat-tabs]")).toBeNull();
  ...
  test("shows the selected model on both transient surfaces when the provider list is not loaded", async () => {
    ...
    const quick = mount(quickPanel(null, () => {}, () => {}, true, providersWithoutModels));
    const side = mount(panel(null, () => {}, () => {}, true, providersWithoutModels));
    await flush();

    expect(quick.container.querySelector('button[title="Model"]')?.textContent)
      .toContain("gpt-test");
    expect(side.container.querySelector('button[title="Model"]')?.textContent)
      .toContain("gpt-test");
  ...
  test("selects an advertised model before the first Quick Chat or Side Chat prompt", async () => {
    ...
    const chooseNextModel = async (view) => {
      const trigger = view.container.querySelector<HTMLButtonElement>('button[title="Model"]');
    ...
    const quick = mount(quickPanel(null, () => {}, () => {}, true, providerList));
    await flush();
    await chooseNextModel(quick);
    quick.unmount();

    const side = mount(panel(null, () => {}, () => {}, true, providerList));
    await flush();
  ...
  test("keeps the complete conversation control row on both transient chat surfaces", async () => {
    ...
    for (const renderPanel of [quickPanel, panel]) {
      ...
      imageInput.click = () => { imagePickerOpens += 1; };
      click(addButton);
      expect(imagePickerOpens).toBe(1);

      expect(button(view.container, "Mode: Ask first")).not.toBeNull();
      expect(button(view.container, "Voice input — hold to talk, or click to toggle dictation"))
        .not.toBeNull();
      expect(button(view.container, "Send message")).not.toBeNull();
      expect(view.container.querySelector('button[title="Model"]')).not.toBeNull();
  ...
  test("uses restrained elevation for the floating panel and its composer", async () => {
    ...
    const composer = view.container.querySelector("[data-transient-chat-composer]");
  ...
  test("moves Quick Chat by its header, clamps it to the viewport, and releases cancellation", async () => {
    ...
    const view = mount(quickPanel());
    await flush();
    const floatingPanel = view.container.querySelector<HTMLElement>(".quick-chat-panel");
    const dragHandle = view.container.querySelector<HTMLElement>("[data-quick-chat-drag-handle]");
    let capturedPointer: number | null = null;

    expect(floatingPanel).not.toBeNull();
    expect(dragHandle).not.toBeNull();
    if (!floatingPanel || !dragHandle) throw new Error("missing Quick Chat drag surface");
    ...
    dragHandle.setPointerCapture = (pointerId) => { capturedPointer = pointerId; };
    ...
    dragHandle.dispatchEvent(new dom.window.PointerEvent("pointerdown", {
      bubbles: true,
      button: 0,
      clientX: 400,
      clientY: 200,
    ...
    dragHandle.dispatchEvent(new dom.window.PointerEvent("pointermove", {
      bubbles: true,
      clientX: 520,
      clientY: 310,
      pointerId: 9,
    ...
    expect(floatingPanel.style.getPropertyValue("--quick-chat-offset-x")).toBe("120px");
    expect(floatingPanel.style.getPropertyValue("--quick-chat-offset-y")).toBe("110px");
    ...
    await flush();
    expect(floatingPanel.style.getPropertyValue("--quick-chat-offset-x")).toBe("272px");
    expect(floatingPanel.style.getPropertyValue("--quick-chat-offset-y")).toBe("132px");

    dragHandle.dispatchEvent(new dom.window.PointerEvent("pointercancel", {
      bubbles: true,
      pointerId: 9,
    }));
    expect(capturedPointer).toBeNull();
    expect(dom.document.body.classList.contains("moving-quick-chat")).toBe(false);

    const headerButton = dragHandle.querySelector<HTMLElement>("button");
    headerButton?.dispatchEvent(new dom.window.PointerEvent("pointerdown", {
      bubbles: true,
      button: 0,
      pointerId: 10,
    }));
  ...
  test("keeps Quick Chat mounted as a nonmodal floating panel while hidden", () => {
    activateDom();
    const view = mount(quickPanel(null, () => {}, () => {}, false));
    const floatingPanel = view.container.querySelector(".quick-chat-panel");
  ...
  test("opens an app-lifetime Quick Chat surface and creates independent tabs", async () => {
    ...
    expect(view.container.querySelector(".quick-chat-panel")?.hasAttribute("data-open")).toBe(true);
  ...
  test("replaces the current Side Chat when a new excerpt arrives instead of hiding extra tabs", async () => {
    ...
    await waitFor(() => {
      expect(view.container.querySelector("textarea")?.value).toBe("First excerpt");
    });
    ...
    await waitFor(() => {
      expect(view.container.querySelector("textarea")?.value).toBe("Second excerpt");
    });
  ...
  test("hides without deleting the current local tab", async () => {
    ...
    dom.window.localStorage.setItem("codetwo.language", "en");
    let closes = 0;
    const view = mount(quickPanel(null, () => { closes += 1; }));
    await waitFor(() => {
      expect(view.container.querySelectorAll('[role="tab"]')).toHaveLength(1);
    });
