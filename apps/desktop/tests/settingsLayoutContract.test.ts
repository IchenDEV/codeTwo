...
describe("Settings page layout contract", () => {
  ...
  test("groups the settings menu and keeps it scrollable in short windows", () => {
    ...
    expect(source).toMatch(
      /<nav[\s\S]*?className="[^"]*\bmin-h-0\b[^"]*\boverflow-y-auto\b[^"]*"/u
    );
  });

  test("matches the settings sidebar to the persisted main rail width", () => {
    expect(appSource).toMatch(
      /<SettingsPage[\s\S]*?sidebarWidth=\{railWidth\}/u
    );
    expect(source).toContain(
      '"--settings-sidebar-width": `${Math.min(420, Math.max(220, sidebarWidth))}px`'
    );
    expect(styles).toMatch(
      /\.settings-sidebar \{[\s\S]*?width: var\(--settings-sidebar-width\);/u
    );
  ...
  test("collapses navigation labels and stacks regular rows when space is constrained", () => {
    ...
    expect(styles).toMatch(/\.settings-sidebar \{[\s\S]*?width: 4rem;/u);
  ...
  test("uses the standard Button for the Back action", () => {
    expect(source).toMatch(
      /<Button[\s\S]*?data-settings-back[\s\S]*?variant="ghost"[\s\S]*?size="row"/u
    );
  });

  test("keeps every settings tab on the shared titlebar", () => {
    expect(source).toMatch(
      /<header[\s\S]*?data-settings-titlebar[\s\S]*?className="[^"]*\bsettings-titlebar\b[^"]*\bshrink-0\b[^"]*"/u
    );
  ...
  test("dismisses the narrow sidebar before opening settings from it", () => {
    expect(appSource).toMatch(
      /onOpenSettings=\{\(\) => \{[\s\S]*?setSettingsInitialTab\("general"\);\s*if \(railOverlay\) setNarrowRailOpen\(false\);\s*setShowSettings\(true\);/u
    );
    expect(appSource).toMatch(
      /onOpenUsage=\{\(\) => \{[\s\S]*?setSettingsInitialTab\("usage"\);\s*if \(railOverlay\) setNarrowRailOpen\(false\);\s*setShowSettings\(true\);/u
    );
  });

  test("includes Usage as a first-class settings panel", () => {
    expect(source).toMatch(
      /\{ id: "usage", icon: ChartNoAxesColumn, labelKey: "usage\.title" \}/u
    );
    expect(source).toContain('{tab === "usage" && (');
    expect(source).toMatch(
      /<UsagePanel[\s\S]*?provider=\{provider\}[\s\S]*?providerNames=\{providerNames\}[\s\S]*?\/>/u
    );
  });

  test("includes Profile as a first-class personal panel", () => {
    expect(source).toMatch(
      /\{ id: "profile", icon: UserRound, labelKey: "profile\.title" \}/u
    );
    expect(source).toContain(
    ...
    expect(styles).toMatch(
      /@container settings-page \(max-width: 40rem\) \{[\s\S]*?\.profile-header \{[\s\S]*?flex-direction: column;[\s\S]*?\.profile-activity-body \{[\s\S]*?grid-template-columns: 1fr;/u
    );
  ...
  test("gives Appearance a spaced flat hierarchy instead of stacked elevation cards", () => {
    ...
    expect(appearanceStyles).toMatch(
      /\.appearance-settings\s*{[^}]*display:\s*flex;[^}]*flex-direction:\s*column;[^}]*gap:\s*var\(--ds-space-page-section\);/su
    );
    expect(appearanceStyles).toMatch(
      /\.appearance-section\s*{[^}]*gap:\s*var\(--ds-space-surface-inset\);/su
    );
    ...
    expect(appearanceStyles).not.toMatch(
      /\.appearance-(?:scheme-option|theme-card)\s*{[^}]*box-shadow:\s*var\(--ds-elevation-surface\)/su
    );
    ...
    expect(compactAppearanceStyles).toMatch(
      /\.appearance-theme-grid\s*{[^}]*repeat\(2, minmax\(0, 1fr\)\)/su
    );
    expect(compactAppearanceStyles).toMatch(
      /\.appearance-palette-grid,\s*\.appearance-profile-grid\s*{[^}]*grid-template-columns:\s*minmax\(0, 1fr\)/su
    );
    expect(appearanceStyles).toMatch(
      /@container \(max-width:\s*30rem\)[\s\S]*?\.appearance-scheme-grid,[\s\S]*?\.appearance-theme-grid\s*{[^}]*minmax\(0, 1fr\)/u
    );
    expect(appearanceStyles).toMatch(
      /@container \(max-width:\s*30rem\)[\s\S]*?\.appearance-scheme-option\s*{[^}]*grid-template-columns:\s*minmax\(0, 9rem\) minmax\(0, 1fr\)/u
    );
  });

  test("includes session import as a first-class personal panel", () => {
    expect(source).toMatch(
      /\{ id: "import", icon: Download, labelKey: "settings\.import" \}/u
    );
  ...
  test("includes Pets as a first-class settings panel", () => {
    expect(source).toMatch(
      /\{ id: "pets", icon: PawPrint, labelKey: "settings\.pets" \}/u
    );
    expect(source).toContain('{tab === "pets" && (');
    expect(source).toMatch(
      /<Page title=\{t\("settings\.pets"\)\}>[\s\S]*?<PetSettings \/>/u
    );
  ...
  test("keeps Settings content clear of the bottom scroll boundary", () => {
    ...
    expect(styles).toMatch(
      /\.settings-page\s*{[^}]*padding:\s*2rem 2rem var\(--ds-space-page-end\);/su
    );
    expect(styles).toMatch(
      /@media \(max-width:\s*44rem\)[\s\S]*?\.settings-page\s*{[^}]*padding:\s*1rem 1rem var\(--ds-space-page-end\);/u
    );
  });
  ...
  test("reuses flat setting rows for the Pets catalog and behavior group", () => {
    ...
    expect(petStyles).not.toMatch(/\.pet-catalog\s*{[^}]*box-shadow:/su);
  ...
  test("removes and closes the Memory tab when its component policy is disabled", () => {
    ...
    expect(source).toMatch(
      /\{tab === "memory" && memoryEnabled && \([\s\S]*?<MemorySettingsPage[\s\S]*?projectPath=\{projectPath\}[\s\S]*?projects=\{projects\}[\s\S]*?onOpenSession=\{onOpenSession\}[\s\S]*?\/>(?:[\s\S]*?)\)\}/u
    );
  });
  ...
  test("groups the Memory library into one neutral workbench", () => {
    ...
    expect(memoryStyles).toMatch(
      /\.memory-workbench\s*{[^}]*overflow:\s*clip;[^}]*background:\s*var\(--background\);/su
    );
    expect(memoryStyles).toMatch(
      /\.memory-detail-panel\s*{[^}]*background:\s*var\(--background\);[^}]*box-shadow:\s*inset/su
    );
    expect(memoryStyles).toMatch(
      /\.memory-management-grid\s*{[^}]*grid-template-columns:\s*minmax\(0, 1\.08fr\) minmax\(20rem, 0\.92fr\);/su
    );
    expect(memoryStyles).toMatch(
      /@media \(max-width:\s*64rem\)[\s\S]*?\.memory-detail-panel\s*{[^}]*display:\s*none;/u
    );
    expect(memoryStyles).toMatch(
      /@media \(max-width:\s*50rem\)[\s\S]*?\.memory-view-switcher > \[data-slot="view-switcher"\]\s*{[^}]*flex-wrap:\s*wrap;/u
    );
    expect(memoryStyles).toMatch(
      /\.memory-view-switcher \[data-slot="button"\]\[data-selected="true"\]::after\s*{[^}]*height:\s*var\(--ds-space-optical\);[^}]*background:\s*var\(--primary\);[^}]*content:\s*"";/su
    );
    expect(memoryStyles).not.toMatch(
      /\.memory-view-switcher \[data-slot="button"\]\[data-selected="true"\]\s*{[^}]*box-shadow:\s*inset/su
    );
  });
