...
describe("composer drafts", () => {
  ...
  test("promotes a project draft without changing identity or overwriting a destination", () => {
    ...
    expect(moved.drafts.get(composerDraftScopeKey(session))?.id).toBe("draft-source");

    drafts = updateComposerDraft(moved.drafts, {
      scope: otherProject,
    ...
    expect(conflict.drafts.get(composerDraftScopeKey(otherProject))?.id).toBe("draft-destination");
    expect(conflict.drafts.get(composerDraftScopeKey(session))?.id).toBe("draft-source");
  });

  test("round-trips minimal attachment references and execution posture", () => {
    const storage = new MemoryStorage();
    const drafts = updateComposerDraft(new Map(), {
      scope: project,
      doc: [
        ...doc,
        { type: "skill", skill_id: "review", params: { focus: "correctness" } },
      ],
      attachments: [{ id: "image-1", kind: "attachment", name: "Diagram.png" }],
      posture,
  ...
  test("rejects corrupt or unavailable storage without throwing", () => {
    ...
    corrupt.setItem(COMPOSER_DRAFT_STORAGE_KEY, JSON.stringify({ version: 1, drafts: [{}] }));
    expect(loadComposerDrafts(corrupt)).toEqual({ drafts: new Map(), warning: "corrupt" });
