...
const t: Translate = (key, values) => `${key}:${values?.count ?? values?.date ?? ""}`
...
function pullRequest(state: SidebarPullRequestStatus["state"]): SidebarPullRequestStatus {
...
describe("TaskBoard workspace model", () => {
  ...
  test("formats each relative-time boundary and the dated fallback", () => {
    ...
    expect(formatUpdatedAt(now - 3_599_999, "en-US", t)).toBe("taskboard.updatedNow:")
    expect(formatUpdatedAt(now - 3_600_000, "en-US", t)).toBe("taskboard.updatedHours:1")
    expect(formatUpdatedAt(now - 86_400_000, "en-US", t)).toBe("taskboard.updatedDays:1")
    expect(formatUpdatedAt(now - 604_800_000, "en-US", t)).toBe("taskboard.updatedOn:Jan 3")
  })
  ...
  test("derives activity without hiding a provider state", () => {
    ...
    expect(sessionActivityKind(session({
      running: true,
      activity: { revision: 1, state: { kind: "failed", reason: "provider_error", message: "bad" } },
    }))).toBe("failed")
  })

  test("uses timestamps and checkout identities in precedence order", () => {
    expect(sessionUpdatedAt(session({ lastActiveAt: 0, createdAt: 2 }), 3)).toBe(0)
    ...
    expect(sessionCheckoutPath(session({ worktreeDiscarded: true, worktreePath: "/w" }))).toBeNull()
    expect(sessionCheckoutPath(session({ worktreePath: "/w", cwd: "/c" }))).toBe("/w")
  ...
  test("maps every activity state to status copy and tone", () => {
    const awaiting = session({
      activity: { revision: 1, state: { kind: "awaiting_input", turn_id: "t", pending: [] } },
    })
  ...
  test("describes attention, failure, and ordinary activity", () => {
    const awaiting = (pending: boolean): SessionProjection => session({
      activity: {
        revision: 1,
        state: {
          kind: "awaiting_input",
          turn_id: "t",
          pending: pending ? [{ input_id: "i", kind: "permission", title: "Choose", options: [], sequence: 1 }] : [],
        },
      },
    })
    expect(sessionActivityDescription(t, awaiting(true))).toBe("Choose")
    expect(sessionActivityDescription(t, awaiting(false))).toBe("taskboard.attention.inputDescription:")
    expect(sessionActivityDescription(t, session({
      activity: { revision: 1, state: { kind: "failed", reason: "interrupted", message: "Stopped" } },
    }))).toBe("Stopped")
  ...
  test("projects newest-first history and selects the latest available session", () => {
    ...
    const [projected] = projectTasks([task({ sessionIds: ["s1", "missing", "s2"] })], sessions)
    expect(projected.sessions.map(({ id, number, current }) => ({ id, number, current }))).toEqual([
    ...
    const [allArchived] = projectTasks([task({ sessionIds: ["s2"] })], sessions)
    expect(allArchived.currentSession).toBeUndefined()
    const activeSessions = new Map([
      ["s1", session({ id: "s1", archived: false })],
      ["s2", session({ id: "s2", archived: false })],
    ])
    const [latestActive] = projectTasks([task({ sessionIds: ["s1", "s2"] })], activeSessions)
  ...
  test("counts only open delivery states for usable checkouts", () => {
    const states: SidebarPullRequestStatus["state"][] = [
      "merged", "closed", "open", "conflicting", "ci_failed", "ci_running",
    ]
    const sessions = states.map((state, index) => session({ id: state, cwd: `/${index}` }))
    sessions.push(session({ id: "none" }), session({ id: "discarded", cwd: "/x", worktreeDiscarded: true }))
  ...
  test("uses the matching pull-request translation key", () => {
    for (const state of ["merged", "conflicting", "ci_failed", "ci_running", "open", "closed"] as const) {
      expect(pullRequestStatusLabel(t, state)).toBe(`rail.pullRequest.${state}:`)
    }
  })
})
