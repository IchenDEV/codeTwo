...
const { FeishuWorkspacePage } =
  await import("../src/feishu/FeishuWorkspacePage");
...
describe("FeishuWorkspacePage", () => {
  test("renders contacts, documents, and bases as one flat directory with real avatars", async () => {
            ...
            chats: [
              {
                ...
                latestMessage: "I updated the launch flow.",
              ...
              },
              {
                ...
                name: "No Avatar",
                description: "Design",
                latestMessage: "Can you review this?",
                avatarUrl: "",
                mode: "p2p",
                type: "p2p",
              },
            ...
            documents: [
              {
                ...
                summary: "Release scope and acceptance criteria",
              },
              {
                id: "doc-2",
                name: "Title only",
    ...
    await waitFor(() =>
      expect(resourceButton(view.navigationHost, "Lin Xiaoman")).not.toBeNull()
    );
    expect(view.container.querySelector(".feishu-workspace")).toBeNull();
    expect(commands).not.toContain("conversation.messages");
    expect(
      view.navigationHost.querySelector("[data-feishu-directory-toggle]")
    ).toBeNull();
    expect(view.navigationHost.querySelector('[role="tablist"]')).toBeNull();
    expect(
      view.navigationHost.querySelectorAll("[data-feishu-section]")
    ).toHaveLength(3);
    ...
    const contactsToggle = view.navigationHost.querySelector(
    ...
    expect(
      resourceButton(view.navigationHost, "Release tracker")
    ).not.toBeNull();
    expect(
      resourceButton(view.navigationHost, "Lin Xiaoman")
        ?.querySelector("img")
        ?.getAttribute("src")
    ).toBe("https://example.invalid/lin.png");
    expect(
      resourceButton(view.navigationHost, "Lin Xiaoman")?.textContent
    ).toContain("I updated the launch flow.");
    expect(
      resourceButton(view.navigationHost, "Launch brief")?.textContent
    ).toContain("Release scope and acceptance criteria");
    expect(
      resourceButton(view.navigationHost, "Launch brief")?.textContent
    ).not.toContain("docx");
    expect(
      resourceButton(view.navigationHost, "Launch brief")?.querySelector("svg")
    ).toBeNull();
    expect(
      resourceButton(view.navigationHost, "Launch brief")?.className
    ).toContain("py-1.5");
    expect(resourceButton(view.navigationHost, "Title only")?.textContent).toBe(
      "Title only"
    );
    const fallback = resourceButton(
      view.navigationHost,
      "No Avatar"
    )?.querySelector("[data-feishu-avatar-fallback]");
  ...
  test("renders Feishu messages as a flat Markdown conversation", async () => {
    ...
    await waitFor(() =>
      expect(text(view.container, "Launch approved")).not.toBeNull()
    );
    ...
    expect(
      message?.querySelector('a[href="https://example.invalid/brief"]')
        ?.textContent
    ).toBe("Open the brief");
    expect(
  ...
  test("renders resolved sender names and avatars instead of internal Feishu ids", async () => {
    ...
    await waitFor(() =>
      expect(text(view.container, "Lin Xiaoman")).not.toBeNull()
    );
    expect(view.container.textContent).not.toContain("Member · 79dcab");
    expect(
      view.container
        .querySelector("[data-feishu-message-avatar] img")
        ?.getAttribute("src")
    ).toBe("https://example.invalid/lin.png");
    view.unmount();
  });
  ...
  test("renders Feishu document content as Markdown", async () => {
    ...
    await waitFor(() =>
      expect(text(view.container, "Launch plan")).not.toBeNull()
    );
    ...
    expect(
      document?.querySelector('a[href="https://example.invalid/source"]')
        ?.textContent
    ).toBe("Open source");
  ...
  test("uses the isolated official Feishu document component before the Markdown fallback", async () => {
    ...
    expect(iframe?.getAttribute("src")).toBe(
      "http://127.0.0.1:43123/component/component-view-1"
    );
    ...
    expect(
      commands.find((command) => command.name === "document.component")?.input
    ).toMatchObject({
    ...
    expect(view.container.textContent).not.toContain(
      "Opening the live Feishu document…"
    );
    expect(
      view.container.querySelector("[data-feishu-document-fallback]")
    ).toBeNull();
    view.unmount();
  });
  ...
  test("collapses sections, limits long resource lists, and keeps pinned resources visible", async () => {
    ...
    await waitFor(() =>
      expect(resourceButton(view.navigationHost, "Chat 1")).not.toBeNull()
    );
  ...
  test("shows local activity dots and refreshes the visible conversation from connector events", async () => {
    ...
    expect(
      view.navigationHost.querySelector(
        '[data-feishu-activity-dot="messages:chat-2"]'
      )
    ).not.toBeNull();
    expect(
      view.navigationHost.querySelector(
        '[data-feishu-section-activity="messages"]'
      )
    ).not.toBeNull();
    expect(
      resourceButton(view.navigationHost, "Background chat")?.textContent
    ).toContain("A new review request");
    ...
    expect(
      view.navigationHost.querySelector(
        '[data-feishu-activity-dot="documents:doc-1"]'
      )
    ).not.toBeNull();
    view.unmount();
  });
  ...
  test("hides resource groups until authorization and routes sign-in through plugin settings", async () => {
    ...
    await waitFor(() =>
      expect(button(english.navigationHost, "Sign in to Feishu")).not.toBeNull()
    );
    expect(
      english.navigationHost.querySelectorAll("[data-feishu-section]")
    ).toHaveLength(0);
    expect(
      english.navigationHost.querySelectorAll("[data-feishu-section-toggle]")
    ).toHaveLength(0);
    expect(text(english.container, "Sign in to Feishu")).not.toBeNull();
    expect(english.container.textContent).not.toContain(
      "Finish connecting your Feishu account"
    );
    expect(
      text(english.settingsHost, "Finish connecting your Feishu account")
    ).not.toBeNull();
    ...
    await waitFor(() =>
      expect(button(chinese.navigationHost, "登录飞书")).not.toBeNull()
    );
    expect(
      chinese.navigationHost.querySelectorAll("[data-feishu-section]")
    ).toHaveLength(0);
  ...
  test("keeps resource sections flat without a duplicate search and exposes permission failures", async () => {
    ...
    expect(
      view.navigationHost.querySelector(
        'input[aria-label="Search chats and resources"]'
      )
    ).toBeNull();
    expect(
      view.navigationHost.querySelector("[data-feishu-resource-types]")
    ).toBeNull();
    expect(
      view.navigationHost.querySelector("[data-feishu-directory-toggle]")
    ).toBeNull();
  ...
  test("continues OAuth automatically after an existing one-click app is upgraded", async () => {
    ...
    const view = renderFeishu(async (name) => {
      ...
      if (name === "connection.begin") {
        ...
        return beginCalls === 1
          ? {
              url: "https://accounts.feishu.cn/page/launcher",
              flow: "registration",
            }
          : {
