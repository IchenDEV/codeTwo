...
describe("SourceControlModal confirmation", () => {
  test("requires the managed destructive dialog before reverting a checkpoint", async () => {
    ...
    const view = mount(
      <SourceControlModal
        cwd="/repo"
        status={{ is_repo: true, branch: "main", ahead: 0, behind: 0, files: [] }}
        statusLoading={false}
        checkpoints={[{ id: "checkpoint-1", commit: "abc123", message: "Before refactor" }]}
    ...
    await waitFor(() => expect(button(dom.document.body, "Revert")).not.toBeNull());
    click(button(dom.document.body, "Revert"));
    expect(reverted).toEqual([]);
    await waitFor(() => expect(dom.document.body.textContent).toContain("Revert checkpoint?"));
