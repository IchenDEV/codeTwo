...
const { TASKBOARD_SNAPSHOT_VERSION, TASKBOARD_STORAGE_KEY, createBoardTask } =
  await import("../src/taskboard/taskBoard");
const { TaskBoardPage } = await import("../src/taskboard/TaskBoardPage");
const { TASKBOARD_VIEW_STORAGE_KEY } =
...
describe("TaskBoardPage rendered", () => {
  test("renders the sidebar recovery action supplied by the persistent shell", async () => {
    ...
    expect(
      view.container.querySelector('button[aria-label="展开侧栏"]')
    ).not.toBeNull();
  });
  ...
  test("renders the accepted flat Task list and persistent inspector", async () => {
    ...
    expect(view.container.textContent).toContain(
      "每个 Session 最多对应一个当前 pull request"
    );
    expect(
      view.container.querySelector("[data-task-board-page]")
    ).not.toBeNull();
    expect(view.container.querySelectorAll("[data-task-column]")).toHaveLength(
      0
    );
    expect(
      view.container.querySelectorAll('[data-slot="status-indicator"]').length
    ).toBeGreaterThan(0);
  ...
  test("switches the shared Task projection between list and board views", async () => {
    ...
    const search = dom.document.body.querySelector(
    ...
    expect(
      view.container.querySelectorAll(
        '[data-task-column] [data-slot="status-indicator"]'
      )
    ).toHaveLength(4);
    const boardScroll = view.container.querySelector(
  ...
  test("keeps a large persisted list progressive on first render", async () => {
    ...
    const view = await renderBoard();
    expect(view.container.querySelectorAll("[data-task-item]")).toHaveLength(
      40
    );
  ...
  test("uses the selected language for workspace chrome and starter content", async () => {
    ...
    expect(view.container.textContent).toContain(
      "Each Session owns at most one current pull request"
    );
  ...
  test("shows newest-first Session history and scopes worktrees and PRs per Session", async () => {
    ...
    await waitFor(() =>
      expect(view.container.textContent).toContain("#102 · 未合并")
    );
    expect(
      view.container.querySelector(".task-board-session-stack")?.className
    ...
    expect(new Set(loadedPaths)).toEqual(
      new Set(["/worktrees/session-old", "/worktrees/session-current"])
    );

    await click(
      button(view.container, "选择 Session 1：历史实现，状态：已完成")
    );
    expect(
      view.container.querySelector('[aria-label="任务检查器"]')?.textContent
    ).toContain("#101 · 未合并");
  ...
  test("moves an Inspector prompt into the selected Session", async () => {
    ...
    const prompt = view.container.querySelector(
      'textarea[placeholder="添加提示并在这个 Session 中继续…"]'
    );
  ...
  test("settles a failed PR lookup as unavailable instead of checking forever", async () => {
    ...
    await waitFor(() =>
      expect(inspector?.textContent).toContain(
        "这个 Session 的检出目录没有当前 pull request。"
      )
    );
  ...
  test("can deliberately continue a linked Task in a fresh Session", async () => {
    ...
    await openMenu(
      view.container.querySelector('[aria-label="任务操作：继续历史任务"]')
    );
  ...
  test("keeps the Inspector integrated and persistent on wide layouts", async () => {
    ...
    expect(
      view.container.querySelector('[aria-label="隐藏检查器"]')
    ).toBeNull();
    expect(
      view.container.querySelector('[aria-label="显示检查器"]')
    ).toBeNull();
    expect(
      dom.document.body.querySelector('[data-slot="dialog-content"]')
    ).toBeNull();
  ...
  test("keeps Task expansion in the narrow list and switches in place to the Inspector", async () => {
    ...
    expect(
      dom.document.body.querySelector('[data-slot="dialog-overlay"]')
    ).toBeNull();
