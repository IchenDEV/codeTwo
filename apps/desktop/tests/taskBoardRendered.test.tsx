...
describe("TaskBoardPage rendered", () => {
  ...
  test("renders the full four-column task board with semantic counts", async () => {
    ...
    const boardContent = view.container.querySelector(
    ...
    expect(columns.every((column) => column.className.includes("flex-1"))).toBe(
      true
    );
    expect(
      columns.every((column) => !column.className.includes("shrink-0"))
    ).toBe(true);
    expect(
      columns.map(
    ...
    expect(view.container.querySelectorAll('[draggable="true"]')).toHaveLength(
      0
    );
  ...
  test("keeps a large persisted board progressive on first render", async () => {
    ...
    expect(
      renderedCounts.reduce((total, count) => total + count, 0)
  ...
  test("projects live sessions into running and attention lanes", async () => {
    ...
    const tasks = [
      createBoardTask(
        { title: "队列任务", status: "todo" },
        { id: "queue", now: 1 }
      ),
      createBoardTask(
        { title: "运行任务", status: "in_progress", sessionIds: ["running"] },
        { id: "running", now: 2 }
      ),
      createBoardTask(
        { title: "提问任务", status: "in_progress", sessionIds: ["question"] },
        { id: "question", now: 3 }
      ),
      createBoardTask(
        { title: "完成任务", status: "done", sessionIds: ["done"] },
        { id: "done", now: 4 }
      ),
    ];
    dom.window.localStorage.setItem(
    ...
    expect(
      view.container.querySelector('[data-task-column="queue"]')?.textContent
    ).toContain("队列任务");
    expect(
      view.container.querySelector('[data-task-column="running"]')?.textContent
    ).toContain("运行任务");
    expect(
      view.container.querySelector('[data-task-column="needs_you"]')
        ?.textContent
    ).toContain("提问任务");
    expect(
      view.container.querySelector('[data-task-column="done"]')?.textContent
    ).toContain("完成任务");
  ...
  test("searches the rendered cards and preserves exactly four columns", async () => {
    ...
    expect(
      view.container.querySelector('[data-task-column="queue"] header')
        ?.textContent
    ).toContain("1/5");
