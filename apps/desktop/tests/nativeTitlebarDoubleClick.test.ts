...
macOSTest("AppKit preserves every supported titlebar double-click action", () => {
  const buildDirectory = mkdtempSync(join(tmpdir(), "codetwo-titlebar-double-click-"));
  const executable = join(buildDirectory, "titlebar-double-click-harness");
  const harness = fileURLToPath(new URL("./native/titlebarDoubleClickHarness.m", import.meta.url));
