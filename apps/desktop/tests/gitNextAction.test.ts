...
import type { GitHubPullRequest, GitStatus, SourceControlInfo } from "../src/bridge";
import {
...
const pullRequest = (overrides: Partial<GitHubPullRequest> = {}): GitHubPullRequest => ({
  number: 42,
...
const input = (overrides: Partial<GitNextActionInput> = {}): GitNextActionInput => ({
  status: status(),
  loading: false,
...
describe("Git next action", () => {
  test("keeps loading and non-repository states explicit and disabled", () => {
    ...
    expect(ids(loading)).toEqual({ primary: "checking", alternatives: [], reason: "checking" });
    expect(loading.primary.disabled).toBe(true);

    const unavailable = resolveGitNextAction(input({ status: status({ is_repo: false }) }));
  ...
  test("prioritizes local files, then unpushed commits, over forge state", () => {
    const dirty = resolveGitNextAction(input({
      status: status({
        ahead: 2,
        files: [{
          path: "src/app.ts",
    ...
    const ahead = resolveGitNextAction(input({
  ...
  test("offers change-request creation only for a capable task worktree", () => {
    ...
    const checkout = resolveGitNextAction(input({ taskWorktree: false, canCleanup: false }));
    expect(ids(checkout)).toEqual({ primary: "up_to_date", alternatives: [], reason: "clean" });

    const missingCli = resolveGitNextAction(input({
      sourceControl: sourceControl({ required_cli_available: false }),
    }));
    expect(ids(missingCli)).toEqual({ primary: "up_to_date", alternatives: [], reason: "clean" });
  });

  test("orders pull-request blockers before review and merge readiness", () => {
    const conflicting = resolveGitNextAction(input({
      pullRequest: pullRequest({ mergeable: "CONFLICTING", merge_state_status: "DIRTY" }),
    }));
    ...
    const failed = resolveGitNextAction(input({
      pullRequest: pullRequest({
    ...
    const requested = resolveGitNextAction(input({
  ...
  test("keeps pending checks, behind branches, and drafts ahead of merge", () => {
    const pending = resolveGitNextAction(input({
      pullRequest: pullRequest({
        checks: [{
          name: "validate",
          status: "IN_PROGRESS",
    ...
    const behind = resolveGitNextAction(input({
    ...
    const draft = resolveGitNextAction(input({ pullRequest: pullRequest({ is_draft: true }) }));
  ...
  test("routes merge-ready and merged worktrees through existing guarded surfaces", () => {
    ...
    const cleanup = resolveGitNextAction(input({
    ...
    const retained = resolveGitNextAction(input({
      pullRequest: pullRequest({ state: "MERGED" }),
      canCleanup: false,
    }));
  ...
  test("never advertises merge while required review or an unknown blocker remains", () => {
    const awaitingReview = resolveGitNextAction(input({
    ...
    const blocked = resolveGitNextAction(input({
      pullRequest: pullRequest({
        review_decision: "APPROVED",
  ...
  test("degrades forge inspection without hiding valid local Git", () => {
    const degraded = resolveGitNextAction(input({
      sourceControl: null,
