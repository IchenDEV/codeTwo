...
How much have you burned, and when does it free up? C2 scans the transcripts your provider CLIs
...
```
  C2 usage — 373 transcripts scanned
...
Windows show raw totals until you tell C2 your plan's budget:
...
- **Codex totals are cumulative.** Codex restates a running `total_token_usage` on every line, so
  C2 takes the maximum per transcript rather than summing (summing over-counts ~100×). Codex
