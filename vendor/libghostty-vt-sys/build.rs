...
fn build_vendored(link_mode: LinkMode) {
    ...
    match link_mode {
        ...
        LinkMode::Static => emit_static_link_metadata(&target, &search_dirs),
    }
    emit_include_metadata(&[include_dir]);
}

fn emit_static_link_metadata(target: &str, search_dirs: &[PathBuf]) {
    if target.ends_with("-msvc") {
        println!("cargo:rustc-link-lib=static=ghostty-vt-static");
    } else {
        if target.contains("windows") {
            normalize_windows_gnu_archive(search_dirs);
        }
...
fn normalize_windows_gnu_archive(search_dirs: &[PathBuf]) {
    let source = search_dirs
        .iter()
        .map(|dir| dir.join("ghostty-vt-static.lib"))
        .find(|path| path.is_file())
        .expect("expected ghostty-vt-static.lib for Windows static linking");
    let target = search_dirs[0].join("libghostty-vt.a");
    std::fs::copy(&source, &target).unwrap_or_else(|error| {
        panic!(
            "failed to normalize {} as {}: {error}",
            source.display(),
            target.display()
        )
    });
